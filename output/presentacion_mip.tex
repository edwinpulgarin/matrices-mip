...
\begin{frame}[plain]
  ...
  Argentina 2021 como ejemplo — Naciones Unidas, \emph{Handbook on Supply, Use and Input--Output Tables}, Series F No.\,74 Rev.\,1
  \end{center}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}{Punto de partida: el COU}
  El COU tiene \textbf{dos clasificaciones distintas}:
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{block}{Oferta $V$ (industria $\times$ producto)}
      ¿Qué produce cada industria? \\ (la tabla ``quién fabrica qué'')
      \end{block}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{block}{Utilización $U$ (producto $\times$ industria)}
  ...
  \vspace{0.5em}
  \begin{center}
  \textbf{Argentina 2021:}\quad $U$ es de dimensión \textbf{222 productos $\times$ 107 industrias} (rectangular).
  \end{center}
  \begin{itemize}
    \item Filas = 222 \textbf{productos} (cereales, semillas, animales vivos\ldots).
    \item Columnas = 107 \textbf{industrias} (Cultivos agrícolas, Cría de animales\ldots).
  \end{itemize}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}{Paso a paso — Ejemplo 1: Cereales (caso concentrado)}
  \textbf{¿Quién produce cereales?} \; La matriz $D$ (de la Oferta) responde:
  \begin{center}
  \textbf{Cereales: 100\,\% lo produce ``Cultivos agrícolas''.}
  \end{center}
  \begin{itemize}
    \item Cuando \emph{cualquier} industria compra cereales, esa compra se atribuye \textbf{completa} a Cultivos agrícolas.
    \item Es una atribución \textbf{1-a-1}, exacta, sin ningún supuesto.
  \end{itemize}
  \vspace{0.5em}
  \begin{block}{Regla de agrupación 222 $\to$ 107}
  Cada compra de un \emph{producto} se atribuye a la \emph{industria que lo produce}.
  De los 222 productos, \textbf{107 se producen $\sim$100\,\% por una sola industria} y \textbf{151 tienen más del 90\,\%}: casi siempre hay un productor claro.
  \end{block}
\end{frame}

% ------------------------------------------------------------------
\begin{frame}{Paso a paso — Ejemplo 2: Maquila (producción secundaria)}
  Algunos productos los fabrican \textbf{varias industrias}. Ej.: \emph{``Servicios de manufactura'' (maquila)}, uso intermedio total $= 274.4$:
  \begin{center}
  \begin{tabular}{lrr}
    \toprule
    ...
    \ldots & \ldots & \ldots \\
    \midrule
    \textbf{Suma} & \textbf{100\,\%} & \textbf{274.4} \\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{frame}{El supuesto y por qué evita negativos}
  ...
  \end{block}
  \begin{itemize}
    \item Para productos \textbf{concentrados} (cereales) no hay supuesto: es exacto.
    \item El supuesto solo aplica a los \textbf{repartidos}, porque el COU no registra a cuál productor le compró cada quien.
  \end{itemize}
...
\begin{frame}{Reconciliación con el COU (la auditoría)}
  Aunque las celdas se reorganizan, \textbf{el total de cada industria cuadra exacto} contra el COU:
  \begin{center}
  \large
  \textbf{Cría de animales (2021), miles de millones \$:}
  \end{center}
  \begin{center}
  \begin{tabular}{lr}
    \toprule
    COU: consumo intermedio (comprador) & \textbf{885.8} \\
    \midrule
    $\Sigma\,Z$ (consumo intermedio doméstico, básico) & 809.8 \\
    $+$ importaciones intermedias & 16.3 \\
    $+$ impuestos y márgenes & 59.8 \\
    \midrule
    \textbf{= Suma} & \textbf{885.8} \quad\color{verdeok}\textbf{(diferencia 0.00)} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.4em}
...
\begin{frame}{Resultados — Argentina (6 años)}
  ...
  \begin{tabular}{lccc}
    ...
    2019 & $108\times108$ & $7\times10^{-16}$ & 1.74 \\
  ...
  \end{center}
  \begin{itemize}
    \item Cada año: un libro Excel con 12 pestañas (Z, vectores, coeficientes, Leontief, balances y \textbf{Auditoría COU}).
