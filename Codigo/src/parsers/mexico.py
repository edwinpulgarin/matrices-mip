"""
Parser de los Cuadros de Oferta y Utilización (COU) de México — INEGI.

Estructura:
    - Base 2013: hasta 232 actividades / productos (clasificación SCIAN)
    - Base 2018: misma desagregación, año base actualizado
    - Una hoja "Oferta" y una "Utilización" (o nombres similares)
    - Unidad: millones de pesos corrientes (MXN)

Fuente: https://www.inegi.org.mx/temas/mip/
"""
...
HOJAS_OFERTA = ['Oferta', 'OFERTA', 'COU Oferta', 'Cuadro de Oferta',
                'oferta', 'Supply', 'CO']
HOJAS_UTIL   = ['Utilización', 'Utilizacion', 'UTILIZACION', 'COU Utilización',
                'Cuadro de Utilización', 'utilizacion', 'Use', 'CU']
...
def _hallar_encabezado(df: pd.DataFrame, min_texto: int = 3) -> int:
...
def _hallar_col_etiq(df: pd.DataFrame, fila_enc: int) -> int:
...
def _hallar_fin_prod(df: pd.DataFrame, fila_dat: int, col_etiq: int) -> int:
    for i in range(fila_dat, len(df)):
        ...
        if any(k in etiq for k in ['valor agreg', 'remuner', 'excedente', 'ingreso mixto',
                                    'impuesto', 'subsidio', 'consumo de capital', 'total']):
            if i > fila_dat + 5:
                return i
    return len(df)


def parsear(ruta: Path, anio: int, base: int = 2013,
            verbose: bool = False) -> COU:
    """
    Lee el COU de México y retorna un objeto COU.

    base : año base (2013 o 2018)
    """
    if verbose:
    ...
    return _parsear_inegi(df_o, df_u, anio, base, verbose)


def _parsear_inegi(df_o, df_u, anio, base, verbose):
